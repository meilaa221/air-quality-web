import { NextResponse } from 'next/server'
import { getReadingsCollection } from '@/lib/mongodb'

export async function GET(request) {
  try {
    const { searchParams } = new URL(request.url)
    const months = parseInt(searchParams.get('months')) || 12

    const readings = await getReadingsCollection()

    const cutoffTime = new Date()
    cutoffTime.setMonth(cutoffTime.getMonth() - months)

    const pipeline = [
      {
        $match: {
          timestamp: { $gte: cutoffTime }
        }
      },
      {
        $group: {
          _id: {
            year: { $year: '$timestamp' },
            month: { $month: '$timestamp' }
          },
          avgPpm: { $avg: '$ppm' },
          maxPpm: { $max: '$ppm' },
          minPpm: { $min: '$ppm' },
          avgTemperature: { $avg: '$temperature' },
          maxTemperature: { $max: '$temperature' },
          minTemperature: { $min: '$temperature' },
          avgHumidity: { $avg: '$humidity' },
          motionCount: {
            $sum: { $cond: [{ $eq: ['$motion', true] }, 1, 0] }
          },
          totalReadings: { $sum: 1 },
          dangerousDays: {
            $sum: { $cond: [{ $gte: ['$ppm', 2000] }, 1, 0] }
          }
        }
      },
      {
        $sort: { '_id.year': 1, '_id.month': 1 }
      },
      {
        $project: {
          _id: 0,
          date: {
            $dateFromParts: {
              year: '$_id.year',
              month: '$_id.month',
              day: 1
            }
          },
          year: '$_id.year',
          month: '$_id.month',
          avgPpm: { $round: ['$avgPpm', 1] },
          maxPpm: { $round: ['$maxPpm', 1] },
          minPpm: { $round: ['$minPpm', 1] },
          avgTemperature: { $round: ['$avgTemperature', 1] },
          maxTemperature: { $round: ['$maxTemperature', 1] },
          minTemperature: { $round: ['$minTemperature', 1] },
          avgHumidity: { $round: ['$avgHumidity', 1] },
          motionCount: 1,
          totalReadings: 1,
          dangerousDays: 1
        }
      }
    ]

    const monthlyStats = await readings.aggregate(pipeline).toArray()

    const monthNames = [
      'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni',
      'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'
    ]

    const formattedStats = monthlyStats.map(stat => ({
      ...stat,
      airQuality: getAirQualityStatus(stat.avgPpm),
      monthName: monthNames[stat.month - 1],
      dateFormatted: `${monthNames[stat.month - 1]} ${stat.year}`
    }))

    return NextResponse.json({
      data: formattedStats,
      count: formattedStats.length,
      period: `${months} months`
    })
  } catch (error) {
    console.error('Error fetching monthly stats:', error)
    return NextResponse.json(
      { error: 'Failed to fetch monthly stats' },
      { status: 500 }
    )
  }
}

function getAirQualityStatus(ppm) {
  if (ppm < 500) return 'Good'
  if (ppm < 1000) return 'Moderate'
  if (ppm < 2000) return 'Poor'
  return 'Dangerous'
}

export const dynamic = 'force-dynamic'
export const revalidate = 0
